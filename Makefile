...
SEMVER_SHORT_SHA1=$(git rev-parse --short HEAD)
SEMVER_MAJOR_MINOR_PATCH=$(bundle exec semver format '%M.%m.%p')
...
BUILD_AMRWB=0 # Disabled for API 23 (Has text relocation issue, don't use when targetting API 23 for now)
BUILD_ZRTP=1
BUILD_SILK=1
BUILD_TUNNEL=0
BUILD_WEBRTC_AECM=1
...
FFMPEG_CONFIGURE_OPTIONS=--target-os=linux --enable-cross-compile --enable-runtime-cpudetect \
	...
	--enable-decoder=h264 --enable-decoder=h263 --enable-encoder=h263 --extra-cflags="-w" \
